...
const SidebarContent = ({ onClose, ...rest }: SidebarProps) => {
  ...
  const { data, error, isLoading } = useSWR<RetailerInterface[]>(
    () => (isTenantUser && session?.user?.tenantId ? `/retailers` : null),
    () => getRetailers({ tenant_id: session?.user?.tenantId }).then(({ data }) => data),
  );
  loading = isLoading;
  ...
  const MockedLinkItems: Array<NavItemPropsInterface> = [
    ...
    {
      name: 'Retailers',
      path: isTenantUser ? `/retailers/view/${data?.[0]?.id}` : '/retailers',
      entity: 'retailer',
    ...
    { name: 'Sales', path: '/sales', entity: 'sale', service: AccessServiceEnum.PROJECT, icon: FiPercent },
