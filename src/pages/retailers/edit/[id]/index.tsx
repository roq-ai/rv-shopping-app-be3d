...
import { getRetailerById, updateRetailerById } from 'apiSdk/retailers';
...
function RetailerEditPage() {
  ...
  const handleSubmit = async (values: RetailerInterface, { resetForm }: FormikHelpers<any>) => {
    ...
    try {
      ...
      const updated = await updateRetailerById(id, values);
      mutate(updated);
      resetForm();
    ...
    >
      <Box rounded="md">
        ...
        <FormWrapper wrapperProps={{ mb: 3 }}>
          <ImagePicker ref={imageUploaderRef} onChange={handleImageChange} entity="retailer" entityId={id} />
        </FormWrapper>
